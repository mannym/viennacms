...
class AdminThemesController extends Controller {
	...
	private function show_theme_thing($name, $theme_info, $default = false, $count = 1) {
		if ($default) {
			$output = '<img src="' . manager::base() . 'layouts/' . $name . '/screenshot.png" style="width: 170px; height: 128px; float: left; margin-right: 5px;" alt="' . $theme_info['name'] . '" />';
			...
			$percentage = floor(98 / (min($count, 3)));
			
			$output = '<div class="available-theme" style="padding-right: 5px; padding-left: 5px; float: left; width: ' . $percentage . '%;">';
