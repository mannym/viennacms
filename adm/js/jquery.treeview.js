		...
		treeview: function(settings) {
			...
			case "location":
				var current = this.find("a").filter(function() {
					var hqs = this.href.split('?');
					hqs = '?' + hqs[1];
//					alert(hqs);
//					alert(location.search);
					var hq = (hqs.length > 1 ? hqs.substring(1).split("&") : []);
					var hrefParts = {};
					
					for(var i = 0; i < hq.length; i++){
						hrefParts[hq[i].match(/^[^=]+/)] = hq[i].replace(/^[^=]+=?/, "");
					}
					...
					for(var i = 0; i < q.length; i++){
						locParts[q[i].match(/^[^=]+/)] = q[i].replace(/^[^=]+=?/, "");
					}
					//					return this.href == location.href; 
//					alert(hrefParts['node']);
					return (locParts['node'] != undefined && hrefParts['node'] == locParts['node']);
				});
				if ( current.length ) {
